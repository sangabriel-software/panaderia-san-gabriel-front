        ...
        <tbody>
          {paginatedOrders.map((order, index) => (
            <tr key={order.idOrdenProduccion} className="align-middle">
              ...
              <td className="text-center p-3">{formatDateToDisplay(order.fechaAProducir)}</td>
              <td className="text-center p-3">{order.cantidadProductos}</td>
              <td className="text-center p-3">
                <Badge pill bg={order.estadoOrden === "C" ? "success" : "warning"} className="px-3 py-2">
                  {order.estadoOrden === "C" ? "Completado" : "Pendiente"}
                </Badge>
              </td>
              <td className="text-center p-3">
                <Button variant="outline-primary" size="sm" onClick={() => onViewDetails(order)} className="me-2">
                  📄 Ver Detalles
                </Button>
                <Button variant="outline-danger" size="sm" onClick={() => onDelete(order.idOrdenProduccion)}>
                  🗑 Eliminar
                </Button>
              </td>
            </tr>
